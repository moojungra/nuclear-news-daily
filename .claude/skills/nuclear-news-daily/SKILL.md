...
4. 최근 7일치 `data/*.json`(오늘 DATE 파일 제외)의 기사 URL을 모아 **중복 제외 목록**으로 쓴다.
...
- **현지어 수집**: `sources.yaml`의 `local` 나라마다 `queries`를 그 나라 말 그대로 검색한다(필요하면 `outlets`를 `allowed_domains`로). 영어권 매체에 아직 안 나온 입찰·정책·여론 기사를 우선 찾는다. 같은 사건의 영문 기사가 있어도 현지 기사가 더 구체적이면 현지 기사를 대표로, 영문은 `related`로.
- 현지어 기사는 본문을 열어 확인한 뒤 한국어로 번역·요약한다. `title`은 원문 제목(원어), `title_ko`는 번역, `language`는 ISO 639-1 코드. 고유명사는 원어나 통용 영문 표기를 괄호로 병기.

## 2. 수집 — 국내

- `sources.yaml`의 `domestic.official`(한수원·한전·정부·원안위 보도자료)과 `domestic.outlets`(에너지 전문지·통신사·경제지)를 대상으로 `domestic.queries`를 한국어로 검색한다. 필요하면 `allowed_domains`로 매체를 좁힌다.
- 공식 보도자료가 있으면 그것을 대표 출처로, 언론 보도는 `related`로.
- 국내 기사 `language`는 `ko`, `title`과 `title_ko`는 같은 값.
- 국내 연예·사회면 수준의 단신(행사 참석, 봉사활동, 인사 동정)은 제외. 단 한전·한수원·원안위 **기관장 인사와 조직 개편**은 포함.
